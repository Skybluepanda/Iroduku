...
async function createRedCard(cid, interaction) {
    ...
    channel.send(`A lucky player got a **Ruby :red_square: ${cid} | ${char.characterName}!**`)
...
async function createDiaCard(cid, interaction) {
    ...
    let channel = interaction.guild.channels.cache.get('948507565577367563');
    channel.send(`A luck sack got a **Diamond :large_blue_diamond: ${cid} | ${char.characterName}!**`)
...
Upload your choice of image using /azuriteupload`).setImage(url)
            ...
            image = database.Image.findOne({where: {imageID: 1}})
            embedCard.addField("no image found", "Upload your choice of image using /azuriteupload");
        }
    } else {
        image = database.Image.findOne({where: {imageID: 1}})
        embedCard.addField("no image found", "Upload your choice of image using /azuriteupload")
...
async function wlPool(interaction) {
    ...
    if (wcount >= 5) {
        ...
        await raritySwitch(cid, 99999, interaction);
    } else {
...
async function majorPool(interaction) {
    ...
    const char = await database.Character.findOne({offset: offset, where: {[Op.lt]: 3}});
    const cid = await char.characterID;
    await raritySwitch(cid, 99999, interaction);
}
