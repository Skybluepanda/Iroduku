...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addIntegerOption(option => 
            option
                .setName("apity")
                .setDescription("Target azurpity")
                .setRequired(true)
                )
    ...
    async execute(interaction) {
        ...
        embedDone.setTitle("Added Apity")
            .setAuthor(interaction.user.username, interaction.user.avatarURL({ dynamic: true }))
            .setColor("#7cff00")
        ...
        try {
            ...
            const quantity = await interaction.options.getInteger('apity');
            const reason = await interaction.options.getString('reason');
            console.log(target);
            console.log(target.username);
            console.log(target.id);
            embedDone.setDescription(`${target.username} updated to ${quantity} apity!
            Reason: ${reason}`);
            await database.Player.update({apity: quantity}, {where: {playerID: target.id}})
