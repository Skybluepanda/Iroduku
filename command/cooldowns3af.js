...
async function createButton2(interaction){
    try {
    const row = new MessageActionRow()
        .addComponents(
            new MessageButton()
    ...
    try {
        ...
        collector.on('collect', async i => {
            switch (i.customId){
                ...
                case 'nope':
                    await nope(interaction, player);
                    await viewCds3(interaction);
                    break;
...
async function nope(interaction, player) {
    const embedDone = new MessageEmbed();
    embedDone.setTitle("Not that one!")
...
function embedG(interaction) {
    const username = interaction.user.username;
    const embedDone = new MessageEmbed();
    embedDone.setTitle("Have a good one!")
            .setAuthor(username, interaction.user.avatarURL({ dynamic: true }))
            .setColor(color.successgreen)
    return embedDone;
};
...
async function viewCds3(interaction) {
    const username = interaction.user.username;
        const userId = interaction.user.id;
        const embedDone = new MessageEmbed();
...
`);
            ...
            const row = await createButton2(interaction);  
            msg = await interaction.followUp({ embeds: [embedDone],components: [row], fetchReply: true });
            await buttonManager(interaction, msg);
        } catch (error) {
            return interaction.followUp({ embeds: [embedError] });
