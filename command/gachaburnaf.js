...
async function createDiaCard(cid, interaction) {
    ...
    channel.send(`A Legendary luck sack got a **Strollarite :diamond_shape_with_a_dot_inside: ${cid} | ${char.characterName} from ${series.seriesName}???**`)
...
async function viewDiaCard(card, interaction) { 
    ...
    if (card.imageID > 0) {
        ...
        if (image) {
            url = await image.imageURL;
            embedCard.setFooter({text: `Art by ${image.artist}
...
async function diacard(card) {
    ...
    if (lock) {
        const lockstatus = '**|**';
        if (tag) {
            const cardString =`:diamond_shape_with_a_dot_inside:` + ID + ` ${lockstatus} ` + tag + charname + ` (#${inumber})`;
            return cardString
        } else {
            const cardString =`:diamond_shape_with_a_dot_inside:` + ID + ` ${lockstatus} ` + charname + ` (#${inumber})`;
            return cardString
        }
    } else {
        const lockstatus = '|';
