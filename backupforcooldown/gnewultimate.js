...
async function mainPool(interaction, rarity) {
    ...
    return await rarityCreate(char.characterID, rarity, interaction);
}
...
async function poolswitch(interaction, rarity) {
    ...
    if (!trashon) {
        return await mainPool(interaction, rarity);
    } else {
        return await allPool(interaction, rarity);
    }
}
...
async function buttonManagerList(msg, embed, interaction, page, timeStart, maxPage) {
    try {   
        ...
        collector.on('collect', async i => {
            switch (i.customId){
                case 'prev':
                    ...
                    await justList(msg, embed, interaction, prevPage, timeStart);
                    break;
                
                case 'next':
                    const nextPage = await checkPage(1, page, maxPage);
                    await justList(msg, embed, interaction, nextPage, timeStart);
                    break;
...
async function returnRarityName(rarity) {
    switch(rarity) {
        ...
        case 2:
            return 'all quartz';
        case 3:
            return `all jade and lesser rarity`;
        case 4:
            return `all lapis and lesser rarity`;
        case 5:
            return `all amethyst and lesser rarity`;
    }
}
...
while burning ${rarityName} cards.`)
